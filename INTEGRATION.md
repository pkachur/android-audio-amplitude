...
`pointDurationMs` продолжает работать — но именно как средняя величина: точка
набирается из целых подокон, и ширины соседних точек могут отличаться примерно
на 5%. Для полоски под плеером это незаметно; если по индексу точки надо
позиционироваться во времени, берите `intervalMs` — там окно ровное.
