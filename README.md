...
Что готово, чем подтверждено и что проверить не удалось — в
[STATUS.md](STATUS.md). Начинать работу над проектом стоит с него.
...
`--points` рассчитан на рисование волны в заданную ширину, а не на точную
временную сетку: точка набирается из целых подокон, и ширины соседних точек
могут отличаться примерно на 5%. Если номер точки пересчитывается во время —
берите `--ms`, там окно ровное по построению.
...
tests/
  ...
  check_eld.py             прогон на реальных записях (десктоп и adb-устройство)
...
STATUS.md                  состояние проекта: что готово и что не проверено
```
...
* декодирование MP3 совпадает с ffmpeg: длина потока — точно, расхождение
  значений — не более 1 LSB (нормальное различие округления для MP3, где
  стандарт описан в плавающей точке). SNR зависит от материала фикстуры:
  71.5 дБ на музыкальной записи, 104.6 дБ на синтетическом сигнале;
...
* код собирается без предупреждений: MSVC 14.44 (`/W4`) и clang 19 из NDK r28c
  под arm64-v8a, armeabi-v7a, x86_64, x86 — и бинарник, и `libamplitude.so`
  (ранее то же подтверждалось clang под Windows). В `.so` проверено через
  `llvm-readelf`: зависимость от `libmediandk.so` есть, `libc++_shared.so` нет
  (libc++ статический), сегменты `LOAD` выровнены на 16 КБ, все четыре
  JNI-символа экспортированы.
...
Полный список того, что осталось непроверенным, — в [STATUS.md](STATUS.md).
